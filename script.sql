DO $$ 
DECLARE
	-- 1. DEclaração de cursor (unbound (não vinculado))
...
-- 	started >= ano AND subscribes >= inscritos;
-- BEGIN
-- 	-- passagem pela ordem 
...
-- DO $$
-- DECLARE
-- 	cur_nomes_e_inscritos CURSOR FOR SELECT youtuber, subscribes FROM tb_top_youtubers;
...
-- cursor unbound com query dinãmica 
-- dinâmica: armazenada como string 
-- exibir canais que começaram somente a partir de um ano especifico 
-- DO $$
-- DECLARE
